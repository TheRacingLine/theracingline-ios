        ...
        ScrollView{
            VStack{
                ...
                GroupBox(label: SettingsLabelView(labelText: "Why so many TBAs?", labelImage: "questionmark.circle")) {
                    Divider().padding(.vertical, 4)
                    SettingsDescriptionView(text: "Many race start times are not published until closer to the event. I include start times as soon as the organisers have published them.")
                } //GROUPBOX
                GroupBox(label: SettingsLabelView(labelText: "Why are some start times not correct?", labelImage: "questionmark.circle")) {
                    Divider().padding(.vertical, 4)
                    SettingsDescriptionView(text: "For events such as NASCAR green flag times are often not published, but TV times are. When there is no clear green flag time, I will use the TV time.").lineLimit(nil)
                } //GROUPBOX
                GroupBox(label: SettingsLabelView(labelText: "Why no rally stage times?", labelImage: "questionmark.circle")) {
                    Divider().padding(.vertical, 4)
                    SettingsDescriptionView(text: "Unfortunately the amount of work required to do this is too high at the moment. I may re-evaluate this as time goes on.")
                } //GROUPBOX
